...
tape('texture validation', function (t) {
  ...
  function comparePixels (expected, tolerance, name) {
  ...
  function checkProperties (texture, props, name) {
    ...
    if ('pixels' in props) {
      createContext.resize(width, height)
      renderTexture({
        texture: texture,
        shape: [width, height]
      })
      comparePixels(props.pixels, props.tolerance || 0, name + ' pixels')
    } else if ('mipmap' in props) {
      var mipmaps = props.mipmap
      for (var i = 0; i < mipmaps.length; ++i) {
        var w = width >> i
        var h = height >> i
        createContext.resize(w, h)
        renderTexture({
          texture: texture,
          shape: [w, h]
        })
        comparePixels(mipmaps[i], props.tolerance || 0, name + ' mipmap ' + i)
      }
    }
    ...
    t.equals(mag, props.mag || gl.NEAREST, name + ' mag filter')
    t.equals(min, props.min || gl.NEAREST, name + ' min filter')
