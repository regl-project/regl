...
module.exports = function wrapAttributeState (
  ...
  function REGLVAO () {
    ...
    if (extension) {
      this.vao = extension.createVetexArrayOES()
    } else {
      this.vao = null
    }
    this.vaoSet[this.id] = this
    this.buffers = []
  }
  ...
  REGLVAO.prototype.bindAttrs = function () {
    ...
    for (var i = 0; i < attributes.length; ++i) {
      ...
      if (attr.buffer) {
        gl.enableVertexAttribArray(i)
        gl.bindBuffer(attr.buffer.type, attr.buffer)
  ...
  REGLVAO.prototype.destroy = function () {
    if (this.vao) {
      ...
      if (this === state.currentVAO) {
        ...
        extensions.bindVertexArrayOES(null)
      }
      extension.deleteVertexArrayOES(this.vao)
      this.vao = null
    }
    if (this.vaoSet[this.id]) {
  ...
  function createVAO (_attr) {
    ...
    function updateVAO (attributes) {
      ...
      check(attributes.length < limits.NUM_ATTRIBUTES, 'too many attributes')
      ...
      for (var i = 0; i < attributes.length; ++i) {
        ...
        if (Array.isArray(spec) || isTypedArray(spec) || isNDArrayLike(spec)) {
          var buf = bufferState.create(spec, 0, false, true)
          ...
          check(rec.dimension >= 1 && rec.dimension <= 4, 'size must be between 1 and 4')
          ...
          check(!!i, 'first attribute must not be a constant')
          rec.x = +spec.x || 0
          rec.y = +spec.y || 0
          rec.z = +spec.z || 0
          rec.w = +spec.w || 0
