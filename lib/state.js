...
module.exports = function wrapContextState (
  ...
  function compileStateScope (defaults, argList, options) {
    ...
    function handleDefaultParam (param, value) {
      ...
      entry.push(STATE_STACK, '.push(', value, ');')
    ...
    // -------------------------------
    var fragSrc = defaults.frag || DEFAULT_FRAG_SHADER
    var vertSrc = defaults.vert || DEFAULT_VERT_SHADER
    if (hasDynamicShader) {
      fragSrc = args.frag || enquote(fragSrc)
      vertSrc = args.vert || enquote(vertSrc)
    ...
    Object.keys(defaultUniforms).forEach(function (uniform) {
      ...
      entry.push(SHADER_STATE, '.uniforms[', NAME, '].push(', VALUE, ');')
      exit.push(SHADER_STATE, '.uniforms[', NAME, '].pop();')
    })
    ...
    Object.keys(defaultAttributes).forEach(function (attribute) {
      ...
      if (typeof data === 'number') {
        entry.push(
          SHADER_STATE, '.pushAttribute(', NAME, ',', +data, ',0,0,0);')
      } else {
        ...
        if (Array.isArray(data)) {
          ...
          entry.push(
            SHADER_STATE, '.pushAttributePointer(', NAME, ',',
            link(buffer), ',',
            offset, ',',
            stride, ',',
            divisor, ',',
      ...
      exit.push(SHADER_STATE, '.popAttribute(', NAME, ');')
    })
    ...
    var procArgs = [
      'return {enter:function(' + entryArgs + '){' +
        entryCode +
      '},exit:function(){' +
        exitCode +
      '},scope:function(BODY,' + entryArgs + '){' +
        entryCode +
        'BODY();' +
        exitCode +
      '},exec:function(' + entryArgs + '){' +
        entryCode +
        exitCode +
