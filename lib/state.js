...
module.exports = function wrapContextState (
  ...
  function compileStateScope (
    ...
    */
    ...
    // entry
    scope.push('function(BODY){')
    scope = scope.concat(entry)

    // execute scoped block
    scope.push('BODY();')

    // exit
    scope = scope.concat(exit)
    scope.push('}')
    ...
    draw.push(GL, '.drawArrays(',
      top(DRAW_STATE.primitives), ',',
      top(DRAW_STATE.offset), ',',
      top(DRAW_STATE.count), ');')

    // exit
    draw = draw.concat(exit)
    draw.push('}')
