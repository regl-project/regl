...
function saveDrawCommandInfo (opts, uniforms, stringStore) {
  saveCommandRef(opts)
  ...
  function addUniforms (dict) {
    Object.keys(dict).forEach(function (u) {
      uniformSet[stringStore.id(u)] = true
    })
  }
  addUniforms(uniforms.static)
  addUniforms(uniforms.dynamic)

  opts._hasCount = 'count' in opts.static || 'count' in opts.dynamic
}
...
function checkDrawCommandState (
  ...
  for (var i = 0; i < progUniforms.length; ++i) {
    var id = progUniforms[i].id
