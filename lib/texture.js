...
function transposeData (image, array, strideX, strideY, strideC, offset) {
  ...
  var n = w * h * c
  var data = pool.allocType(image.type, n)

  var p = 0
  for (var i = 0; i < h; ++i) {
    for (var j = 0; j < w; ++j) {
      for (var k = 0; k < c; ++k) {
        data[p++] = array[strideX * i + strideY * j + strideC * k + offset]
      }
    }
  }
...
module.exports = function createTextureSet (
  ...
  function parseFlags (flags, options) {
    ...
    if ('format' in options) {
      ...
      if (internalformat in textureTypes) {
        if (!('type' in options)) {
          flags.type = textureTypes[internalformat]
        }
      }
  ...
  function setSubImage (info, target, x, y, miplevel) {
    ...
    if (element) {
      gl.texSubImage2D(
        target, miplevel, x, y, width, height, format, format, type, element)
    } else if (info.compressed) {
      gl.compressedTexSubImage2D(target, miplevel, x, y, internalformat, width, height, 0, data)
    } else if (info.needsCopy) {
      reglPoll()
      gl.copyTexSubImage2D(
        target, miplevel, x, y, format, info.copyX, info.copyY, width, height, 0)
    } else {
      gl.texSubImage2D(
        target, miplevel, x, y, format, width, height, 0, format, type, data)
    }
  }
  ...
  function parseMipMapFromObject (mipmap, options) {
    ...
    if (isPixelData(options)) {
      ...
      if (Array.isArray(options.mipmap)) {
        ...
        imgData = mipmap.images[0] = allocImage()
        copyFlags(imgData, mipmap)
        parseImage(imgData, options.data)
        mipmap.mipmask = 1
      } else {
        imgData = mipmap.images[0] = allocImage()
        copyFlags(imgData, mipmap)
  ...
  function parseTexInfo (info, options) {
    ...
    if ('mipmap' in options) {
      switch (typeof options.mipmap) {
        ...
        case 'boolean':
          info.genMipmaps = options.mipmap
          break
  ...
  function createTexture (options) {
    ...
    function reglTexture2D (a, b) {
      ...
      if (texInfo.genMipmaps) {
        texture.mipmaps = (mipData.width << 1) - 1
      }

      check.texture2D(texInfo, mipData)
    ...
    function subimage (image, x, y, level) {
      var imageData = allocImage()
      parseImage(imageData, image)

      tempBind()
      setSubImage(imageData, GL_TEXTURE_2D, x | 0, y | 0, level | 0)
      tempRestore()
  ...
  return {
    create2D: createTexture,
    createCube: createTexture,
