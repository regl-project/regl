    ...
    Object.keys(options).forEach(function (option) {
      ...
      switch (option) {
        ...
        case 'blend.func':
          ...
          batch(
            GL, '.blendEquationSeparate(',
            BLEND_FUNCS, '[', VALUE, '.srcRGB||', VALUE, '.src],',
            BLEND_FUNCS, '[', VALUE, '.srcAlpha||', VALUE, '.src],',
            BLEND_FUNCS, '[', VALUE, '.dstRGB||', VALUE, '.dst],',
            BLEND_FUNCS, '[', VALUE, '.dstAlpha||', VALUE, '.dst]);')
          break
    ...
    Object.keys(staticOptions).forEach(function (param) {
      ...
      switch (param) {
        ...
        case 'blend.func':
          ...
          var srcRGB = value.srcRGB || value.src
          var srcAlpha = value.srcAlpha || value.src
          var dstRGB = value.dstRGB || value.dst
          var dstAlpha = value.dstAlpha || value.dst
      ...
      switch (param) {
        ...
        case 'blend.func':
          ...
          dynamicEntry(
            ...
            BLEND_FUNCS, '[', variable, '.srcRGB||', variable, '.src],',
            BLEND_FUNCS, '[', variable, '.srcAlpha||', variable, '.src],',
            BLEND_FUNCS, '[', variable, '.dstRGB||', variable, '.dst],',
            BLEND_FUNCS, '[', variable, '.dstAlpha||', variable, '.dst]);')
