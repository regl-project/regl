  ...
  function compileCommand (
    ...
    Object.keys(staticOptions).forEach(function (param) {
      ...
      switch (param) {
        ...
        case 'scissor.shape':
          check.type(value, 'object', param)
          var scissorX = value.x || 0
          var scissorY = value.y || 0
          var scissorW = 'w' in value ? value.w : -1
          var scissorH = 'h' in value ? value.h : -1
          check(typeof scissorX === 'number' && scissorX >= 0, 'scissor.x')
          check(typeof scissorY === 'number' && scissorY >= 0, 'scissor.y')
          check(typeof scissorW === 'number' && scissorW >= 0, 'scissor.w')
          check(typeof scissorH === 'number' && scissorH >= 0, 'scissor.h')
          var SCISSOR_SHAPE_STACK = linkContext(param)
          entry(SCISSOR_SHAPE_STACK, '.push(',
            scissorX, ',',
            scissorY, ',',
            scissorW, ',',
            scissorH, ');')
          exit(SCISSOR_SHAPE_STACK, '.pop();')
          break
    ...
    Object.keys(dynamicOptions).forEach(function (param) {
      ...
      switch (param) {
        ...
        case 'scissor.shape':
          var SCISSOR_SHAPE_STACK = linkContext(param)
          dynamicEntry(SCISSOR_SHAPE_STACK, '.push(',
            variable, '.x||0,',
            variable, '.y||0,',
            '"w" in ', variable, '?', variable, '.w:-1,',
            '"h" in ', variable, '?', variable, '.h:-1);')
          dynamicExit(SCISSOR_SHAPE_STACK, '.pop();')
          break
