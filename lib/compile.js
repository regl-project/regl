  ...
  function compileCommand (options, attributes, uniforms, context, hasDynamic) {
    ...
    if (hasDynamicFramebuffer) {
      ...
      entry(
        ...
        widthPrefix, CONTEXT, '.drawingBufferWidth;',
        heightPrefix, CONTEXT, '.drawingBufferHeight;}')
      dynamicPoll(FRAMEBUFFER_STATE, '.poll();')
    } else {
      staticPoll(FRAMEBUFFER_STATE, '.poll();')
    }
    ...
    // Insert poll calls into draw and batch commands
    Object.keys(glState.next).forEach(function (param) {
      if (param === 'framebuffer' || param === 'vert' || param === 'frag') {
        return
      }
    ...
    Object.keys(staticOptions).forEach(function (param) {
      ...
      switch (param) {
        ...
        case 'elements':
          ...
          if (!hasCount) {
            saveProp(DRAW_STATE, '.vertCount')
          }
          if (!hasPrimitive) {
            saveProp(DRAW_STATE, '.primType')
          }
          setStatic(ELEMENT_STATE, '.elements', ELEMENTS)
          if (!hasCount || !hasPrimitive) {
            entry('if(', ELEMENTS, '){')
            if (!hasCount) {
              entry(DRAW_STATE, '.count=', ELEMENTS, '.vertCount;')
            }
            if (!hasPrimitive) {
              entry(DRAW_STATE, '.primitive=', ELEMENTS, '.primType;')
            }
            entry('}else{')
            if (!hasCount) {
              entry(DRAW_STATE, '.count=-1;')
            }
            if (!hasPrimitive) {
              entry(DRAW_STATE, '.primitive=4;')
            }
            entry('}')
          }
          break
      ...
      switch (param) {
        case 'elements':
          ...
          var hasCount = !(
            ...
            ('count' in dynamicOptions))
          if (!hasCount) {
            saveDynamic(DRAW_STATE, '.vertCount')
          }
          if (!hasPrimitive) {
            saveDynamic(DRAW_STATE, '.primType')
          }
          saveDynamic(ELEMENT_STATE, '.elements')
          dynamicEntry(ELEMENT_STATE, '.elements=', value, ';')
          if (!hasCount || !hasPrimitive) {
            dynamicEntry('if(', value, '){')
            if (!hasCount) {
              dynamicEntry(DRAW_STATE, '.count=', value, '.vertCount;')
            }
            if (!hasPrimitive) {
              dynamicEntry(DRAW_STATE, '.primitive=', value, '.primType;')
            }
            dynamicEntry('}else{')
            if (!hasCount) {
    ...
    Object.keys(staticAttributes).forEach(function (attribute) {
      // TODO need to implement attribute parsing here
    })
