...
module.exports = function reglCore (
  ...
  function compileDrawCommand (
    env, block, program, options, uniforms, attributes) {
    var entry = env.block()
    var exit = env.block()
    block(entry, exit)

    var attributeDefs = env.attributeDefs
  ...
  // ===================================================
  // ===================================================
  function compileBatchCommand (env, batch, program, options, uniforms, attributes) {
  ...
  function compileCommand (options, attributes, uniforms, context) {
    ...
    */
    ...
    var definedAttributes = env.attributeDefs = {}
    ...
    // --------------------------------
    Object.keys(definedAttributes).forEach(function (attribute) {
      var defn = definedAttributes[attribute]
    ...
    // -----------------------------
    if (staticShader) {
      compileDrawCommand(env, draw, program, options, uniforms, attributes)
    } else {
      draw(link(function (program, obj, props) {
        ...
        if (!result) {
          ...
          compileDrawCommand(
            dynEnv, dynBlock, program, options, uniforms, attributes)
    ...
    batch.arg()
    if (staticShader) {
      compileBatchCommand(env, batch, program, options, uniforms, attributes)
    } else {
