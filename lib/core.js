...
module.exports = function reglCore (
  ...
  var extDisjointTimerQuery = extensions.ext_disjoint_timer_query
  ...
  function injectExtensions (env, scope) {
    ...
    // inject timer extension here.
  }

  function emitBeginTimerQuery (env, draw) {
    var stats = env.stats
    var EXT_DISJOINT_TIMER_QUERY
    var shared = env.shared

    if (extDisjointTimerQuery) {
      EXT_DISJOINT_TIMER_QUERY = draw.def(shared.extensions, '.ext_disjoint_timer_query')

      // begin timer query.
      draw(EXT_DISJOINT_TIMER_QUERY, '.beginQueryEXT(',
           EXT_DISJOINT_TIMER_QUERY, '.TIME_ELAPSED_EXT,', // target
           stats, '.', '_queries[', stats, '.iQuery', ']',                     // query
           ');')
    }
  }

  function emitEndTimerQuery (env, draw) {
    var stats = env.stats
    var EXT_DISJOINT_TIMER_QUERY
    var shared = env.shared
    ...
    if (extDisjointTimerQuery) {
      ...
      // end timer query.
      draw(EXT_DISJOINT_TIMER_QUERY, '.endQueryEXT(',
           EXT_DISJOINT_TIMER_QUERY, '.TIME_ELAPSED_EXT', // target
           ');')

      // collect timer query information of previous frame.
      draw(
      ...
      draw(stats, '.iQuery', '=(', stats, '.iQuery+1)%2;')
      draw(stats, '.iCollect', '=(', stats, '.iCollect+1)%2;')
    }
  }
