    ...
    Object.keys(dynamicOptions).forEach(function (param) {
      ...
      function handleDynamic (x) {
        ...
        if (Array.isArray(x)) {
          ...
          batch(GL, '.', GL_VARIABLES[name], '(', NEXT, ');')
  ...
  function compileCommand (options, attributes, uniforms, context) {
    ...
    */
    ...
    Object.keys(definedAttributes).forEach(function (attribute) {
      ...
      var binding = env.scopeAttrib(defn.id)
      var record = defn.append(env, scope)
      Object.keys(new AttributeRecord()).forEach(function (key) {
        setScope(binding, '.' + key, record[key])
      })
      if (record.isStream) {
        scopeExit(
          'if(', record.isStream, ')',
