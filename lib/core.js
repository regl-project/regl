...
module.exports = function reglCore (
  ...
  function emitCommon (env, scope, args) {
    if (extInstancing) {
      env.instancing = scope.def(
        env.shared.extensions, '.angle_instanced_arrays')
    }

    emitHeader(env, scope, args)
    emitPollFramebuffer(env, scope)

    emitPollState(env, scope, args)
    emitSetOptions(env, scope, args.state, function (defn) {
      return defn.static
    })

    var program = args.shader.progVar.append(env, scope)
    scope(env.shared.gl, '.useProgram(', program, '.program);')
    return program
  }

  function emitAttributes (env, scope, args, attributes, batchStatic) {
    var shared = env.shared
    ...
    attributes.forEach(function (attribute) {
      ...
      var record
      if (arg) {
        if (arg.batchStatic !== batchStatic) {
          return
        }
  ...
  function emitUniforms (env, scope, args, uniforms, batchStatic) {
    var shared = env.shared
    var GL = shared.gl

    function setUniformLiteral (type, location, value, name) {
      var infix
      switch (type) {
        case GL_FLOAT:
      ...
      scope(GL, '.uniform', infix, '(', location, ',', value.join(), ');')
    }
    ...
    uniforms.forEach(function (uniform) {
      ...
      if (arg) {
        ...
        if (arg.static) {
          ...
          if (type === GL_SAMPLER_2D || type === GL_SAMPLER_CUBE) {
            ...
            setUniformLiteral(type, LOCATION, value, name)
          }
          return
        } else {
      ...
      check.optional(function () {
        ...
        function check (pred) {
          scope(
            'if(', pred, ')',
            CHECK, '.commandRaise(', MESSAGE, ',', COMMAND, ');')
        }
  ...
  function emitDraw (env, outer, inner, args) {
    ...
    function emitCount () {
      ...
      if (defn) {
        ...
        check.optional(function () {
          ...
          var COMMAND = shared.command
          if (defn.MISSING) {
            outer(CHECK, '.commandRaise("missing vertex count",', COMMAND, ');')
          }
          if (defn.DYNAMIC) {
            scope(CHECK, '.command(', COUNT, '>=0,"missing vertex count",', COMMAND, ');')
          }
        })
      } else {
    ...
    if (extInstancing) {
      INSTANCES = emitValue(INSTANCES)
      EXT_INSTANCING = env.instancing
    }

    var ELEMENT_TYPE = ELEMENTS + '.type'

    var elementsStatic = drawOptions.elements && drawOptions.elements.static

    function emitInstancing () {
      function drawElements () {
        draw(EXT_INSTANCING, '.drawElementsInstancedANGLE(',
          [PRIMITIVE, COUNT, ELEMENT_TYPE, OFFSET, INSTANCES], ');')
      }

      function drawArrays () {
        draw(EXT_INSTANCING, '.drawArraysInstancedANGLE(',
    ...
    function emitRegular () {
      function drawElements () {
        draw(GL + '.drawElements(' + [PRIMITIVE, COUNT, ELEMENT_TYPE, OFFSET] + ');')
      }

      function drawArrays () {
        draw(GL + '.drawArrays(' + [PRIMITIVE, OFFSET, COUNT] + ');')
      }
  ...
  function emitDrawBody (env, draw, args, program) {
    emitAttributes(env, draw, args, program.attributes, true)
    emitAttributes(env, draw, args, program.attributes, false)
    emitUniforms(env, draw, args, program.uniforms, true)
    emitUniforms(env, draw, args, program.uniforms, false)
    emitDraw(env, draw, draw, args)
  }

  function emitDrawProc (env, args) {
    var draw = env.proc('draw', 1)
    var program = emitCommon(env, draw, args)

    emitSetOptions(env, draw, args.state, function (defn) {
      return !defn.static
    })
    ...
    if (args.shader.program) {
      ...
      draw(env.link(function (props, program) {
  ...
  function emitBatchBody (env, scope, args, program) {
    ...
    emitSetOptions(env, outer, args.options, function (defn) {
      return !defn.static && defn.batchStatic
    })
    emitSetOptions(env, loop, args.options, function (defn) {
      return !defn.static && !defn.batchStatic
    })
    emitAttributes(env, outer, args, program.attributes, true)
    emitAttributes(env, loop, args, program.attributes, false)
    emitUniforms(env, outer, args, program.uniforms, true)
    emitUniforms(env, loop, args, program.uniforms, false)
    emitDraw(env, outer, loop, args)
  }

  function emitBatchProc (env, args) {
    var batch = env.proc('batch', 2)
    var PROGRAM = emitCommon(env, batch, args)

    if (args.shader.program) {
      emitBatchBody(env, batch, args, args.shader.program)
    } else {
      batch(env.link(function (props, count, program) {

      }), '.call(this,a0,a1,', PROGRAM, ');')
    }
  }
