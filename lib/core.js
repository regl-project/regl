...
module.exports = function reglCore (
  ...
  function emitFramebufferViewportScissor (env, entry, dynamicEntry, batch, draw, saveAll, setAll, options) {
    ...
    function emitFramebufferParse () {
      if (S_FRAMEBUFFER in staticOptions) {
        ...
        return {
          static: false,
          value: invoke(entry, dynamicOptions[S_FRAMEBUFFER])
        }
      }
    }

    var FRAMEBUFFER = emitFramebufferParse()
    ...
    function emitFramebufferSetCond (block, value) {
      block('if(', FRAMEBUFFER.value, '){')
      emitFramebufferSet(block, FRAMEBUFFER.value)
      block('}else{')
      emitFramebufferNull(block)
      block('}')
    }
    ...
    // -------------------------------
    function emitSetBox (param) {
      var NEXT = NEXT_VARS[param]
      var box
      ...
      function parseStaticCorner (name, i) {
        if (name in box) {
          check.command(
            typeof box[name] === 'number' && box[name] >= 0,
            param)
          entry(NEXT, '[', i, ']=', box[name], ';')
        } else {
          entry(NEXT, '[', i, ']=0;')
        }
      }

      function parseStaticDim (name, i) {
        if (name in box) {
          check.command(
            typeof box[name] === 'number' && box[name] >= 0,
            param)
          ...
          entry(NEXT, '[', i, ']=', CONTEXT, '.',
            name === 'w' ? S_FRAMEBUFFER_WIDTH : S_FRAMEBUFFER_HEIGHT, ';')
        }
      }

      function parseDynamicCorner (name, i) {
        entry(NEXT, '[', i, ']=', box, '.', name, '|0;')
      }

      function parseDynamicDim (name, i) {
        entry(
          NEXT, '[', i, ']="', name, '" in ', box,
          '?', box, '.', name,
          ':', CONTEXT, '.',
      ...
      function updateViewport () {
        env.stateDirty = true
        if (param === S_VIEWPORT) {
          setAll(CONTEXT, '.' + S_VIEWPORT_WIDTH, NEXT + '[2]')
          setAll(CONTEXT, '.' + S_VIEWPORT_HEIGHT, NEXT + '[3]')
        }
      }

      if (param in staticOptions) {
        box = staticOptions[param]
        check.commandType(box, 'object', param)
        emitSave()
        parseStaticCorner('x', 0)
        parseStaticCorner('y', 1)
        parseStaticDim('w', 2)
        parseStaticDim('h', 3)
        updateViewport()
      } else if (param in dynamicOptions) {
        box = invoke(entry, dynamicOptions[param])
        emitSave()
        parseDynamicCorner('x', 0)
        parseDynamicCorner('y', 1)
        parseDynamicDim('w', 2)
        parseDynamicDim('h', 3)
        updateViewport()
      } else if (FRAMEBUFFER) {
    ...
    emitSetBox(S_VIEWPORT)
    emitSetBox(S_SCISSOR_BOX)
  }

  function emitStaticOptions (env, batch, draw, setScope, setAll, staticOptions, drawOptions, staticShader) {
    var block = env.block
    ...
    Object.keys(staticOptions).forEach(function (param) {
      ...
      switch (param) {
        ...
        // Fall through here, need to check instancing
        case S_INSTANCES:
        case S_COUNT:
        case S_OFFSET:
  ...
  function emitDynamicOptions (env, entry, dynamicEntry, draw, batch, scope, setAll, setScope, dynamicOptions) {
    var invoke = env.invoke
    var block = env.block
  ...
  function emitOptions (env, entry, dynamicEntry, batch, draw, scope, setScope, setAll, options, drawOptions, staticShader) {
    var staticOptions = options.static
    var dynamicOptions = options.dynamic

    emitStaticOptions(env, batch, draw, setScope, setAll, staticOptions, drawOptions, staticShader)
    emitDynamicOptions(env, entry, dynamicEntry, draw, batch, scope, setAll, setScope, dynamicOptions)
    emitPollOptions(env, draw, batch, options)
  }

  function emitParseDrawOptions (env, entry, staticShader, drawOptions, options) {
    var invoke = env.invoke
    var link = env.link
    ...
    if (S_PRIMITIVE in dynamicOptions) {
      var dyn = dynamicOptions.primitive
      if (batchConstant(dyn) && staticShader) {
        var ref = invoke(entry, dyn)
        var result = entry.def(GL_TYPES, '[', ref, ']')
        drawOptions.primitive = {
          static: true,
          value: result
        }
      } else {
    ...
    ;[ S_INSTANCES, S_COUNT, S_OFFSET ].forEach(function (param) {
    ...
    if (S_ELEMENTS in staticOptions) {
      ...
      drawOptions[S_ELEMENTS] = {
        static: true,
        value: elements ? link(elements) : null
      }
    } else if (S_ELEMENTS in dynamicOptions) {
      drawOptions[S_ELEMENTS] = {
        static: false,
        value: dynamicOptions[S_ELEMENTS]
      }
    }
    if (drawOptions[S_ELEMENTS] && !drawOptions[S_OFFSET]) {
      drawOptions[S_OFFSET] = {
        static: true,
        value: 0
      }
    }
