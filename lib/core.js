...
//    shared[prop] = global.def(SHARED, '.', prop)
    ...
    // so we bascially do
    // STATS = env.link(stats).
    // then env.stats = STATS, so that it is easy to access.
    // then STATS will be something like g4, and we can just use that when generating code.
