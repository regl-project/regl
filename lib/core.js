...
module.exports = function reglCore (
  ...
  function compileCommand (options, attributes, uniforms, context) {
    ...
    var UNIFORM_STATE = shared.uniforms
    ...
    if (S_ELEMENTS in staticOptions) {
      ...
      if (!drawOptions[S_COUNT]) {
        drawOptions[S_COUNT] = {
          static: true,
          value: elements ? elements.vertCount : 0
        }
      }
      if (!drawOptions[S_PRIMITIVE]) {
        drawOptions[S_PRIMITIVE] = {
          static: true,
          value: elements ? elements.primType : GL_TRIANGLES
        }
      }
    ...
    // emit draw commands for scope
    Object.keys(drawOptions).forEach(function (param) {
      var defn = drawOptions[param]
      var value = defn.value
      if (param === S_ELEMENTS) {
        var elements = emitElements(env, scope, scopeExit, value)
        setScope(
          DRAW_STATE,
          '.' + param,
          elements)
