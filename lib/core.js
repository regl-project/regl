...
var S_ENABLE_GPU_TIMER = 'enableGpuTimer'
...
module.exports = function reglCore (
  ...
  function createStaticEnableGpuTimer(value) {
    check.commandType(value, 'boolean', S_ENABLE_GPU_TIMER)
    console.log("PARSE")
    return createStaticDecl(function () { return value })
  }
  var enableGpuTimerDefault = {v: 'true' }
      //createStaticEnableGpuTimer(true) //'true'
  ...
  var sharedState = {
    ...
    timer: timer,
    enableGpuTimerDefault: enableGpuTimerDefault,
  ...
  function parseOptions (options) {
    ...
    var enableGpuTimer
    if(S_ENABLE_GPU_TIMER in staticOptions) {
      console.log("PASEfrefrfr")
      enableGpuTimer = createStaticEnableGpuTimer(staticOptions[S_ENABLE_GPU_TIMER])
    } else if(S_ENABLE_GPU_TIMER in dynamicOptions) {

      var dyn = staticOptions[S_ENABLE_GPU_TIMER]

      enableGpuTimer = createDynamicDecl(dyn, function (env, scope) {
        var DYN = env.invoke(scope, dyn)

        check.optional(function () {
          env.assert(scope,
                     'typeof ' + DYN + '==="boolean"',
                     'invalid flag ' + S_ENABLE_GPU_TIMER)
        })

        return DYN
      })
    } else {
      enableGpuTimer = null; //enableGpuTimerDefault
    }
  ...
  function parseArguments (options, attributes, uniforms, context) {
    ...
    console.log("result: ", result.enableGpuTimer)
  ...
  function emitEnableGpuTimer (env, scope, args) {
    var defn = args.enableGpuTimer
    if(defn)
      env.shared.enableGpuTimer = defn.append(env, scope)
  }
  ...
  function emitBeginTimerQuery (env, draw, args) {
    var STATS = env.stats
    var shared = env.shared
    var ENABLE_GPU_TIMER = shared.enableGpuTimer

    if (timer) {
      var TIMER = shared.timer
      var defn = args.enableGpuTimer

      if(args.enableGpuTimer === null) {
        draw('if(', shared.enableGpuTimerDefault,  '.v){',
             TIMER, '.beginQuery(', STATS, ');',
             '}'
            )
      } else if (isStatic(defn)) {
        if (ENABLE_GPU_TIMER) {
        ...
        /*
          do a cond to execute beginQuery based on value of enable_gpu_timer

          do same for defualt guy.
          */
  ...
  function emitEnableGpuTimer (env, scope, args) {
    var defn = args.enableGpuTimer
    ENABLE_GPU_TIMER = defn.append(env, scope)
  }
  */
  ...
  function emitDrawProc (env, args) {
    ...
    emitEnableGpuTimer(env, draw, args)
  ...
  function emitBatchProc (env, args) {
    ...
    emitEnableGpuTimer(env, batch, args)
  ...
  function emitScopeProc (env, args) {
    ...
    var ENABLE_GPU_TIMER_DEFAULT
    var ENABLE_GPU_TIMER_DEAULT_TEMP
    var defn = args.enableGpuTimer
    ...
    if (timer && defn) {
      ...
      ENABLE_GPU_TIMER_DEFAULT = shared.enableGpuTimerDefault
      ENABLE_GPU_TIMER_DEAULT_TEMP = scope.def(ENABLE_GPU_TIMER_DEFAULT,'.v')

      var AAA = defn.append(env, scope) + ''

//      emitEnableGpuTimer(env, scope, args)
      scope(ENABLE_GPU_TIMER_DEFAULT, ".v=", /*env.shared.enableGpuTimer*/ AAA, ";")

      scope.exit(ENABLE_GPU_TIMER_DEFAULT, '.v=', ENABLE_GPU_TIMER_DEAULT_TEMP, ";")

    }

//    scope(ENABLE_GPU_TIMER_DEFAULT, "=", env.shared.enableGpuTimer, ";")

//    console.log("var: ", ENABLE_GPU_TIMER_DEFAULT)
    /*
      var defn = args.enableGpuTimer
