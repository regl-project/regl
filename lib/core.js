...
module.exports = function reglCore (
  ...
  function emitAttributes (env, scope, args, attributes, filter) {
    ...
    attributes.forEach(function (attribute) {
      ...
      var record
      if (arg) {
        if (!filter(arg.batchStatic, false)) {
          return
        }
        ...
        env.cond(CACHED_PROC)
          ...
          .else(
            ...
            env.link(function (program) {
              var tempEnv = createREGLEnvironment()
              var tempProc = tempEnv.proc('drawBody', 2)
              check.optional(function () {
                tempEnv.command = env.command
                tempEnv.assert = env.assert
              })
              emitDrawBody(tempEnv, tempProc, args, program)
              return tempEnv.compile().drawBody
            }), '(', program, ');',
  ...
  function emitBatchBody (env, scope, args, program) {
    ...
    emitAttributes(env, outer, args, program.attributes,
      function (defn, scope) {
        return scope || defn.batchStatic
      })
    emitAttributes(env, loop, args, program.attributes,
      function (defn, scope) {
        return !scope && !defn.batchStatic
      })
    emitUniforms(env, outer, args, program.uniforms,
      function (defn, scope) {
        return scope || defn.batchStatic
      })
    emitUniforms(env, loop, args, program.uniforms,
      function (defn, scope) {
        return !scope && !defn.batchStatic
      })
      ...
      batch(
        env.cond(CACHED_PROC)
          ...
          .else(
            ...
            env.link(function (program) {
              var tempEnv = createREGLEnvironment()
              var tempProc = tempEnv.proc('batchBody', 2)
              check.optional(function () {
                tempEnv.command = env.command
                tempEnv.assert = env.assert
              })
              emitBatchBody(tempEnv, tempProc, args, program)
              return tempEnv.compile().batchBody
            }), '(', PROGRAM, ');',
