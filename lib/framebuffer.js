...
module.exports = function wrapFBOState (
  ...
  function createFBO (options) {
    ...
    function reglFramebuffer (input) {
      ...
      for (i = 0; i < colorBuffers.length; ++i) {
        attach(GL_COLOR_ATTACHMENT0 + i, colorBuffers[i])
      }
      for (i = colorBuffers.length; i < limits.maxColorAttachments; ++i) {
        attach(GL_COLOR_ATTACHMENT0 + i, null)
      }
      attach(GL_DEPTH_ATTACHMENT, depthBuffer)
      attach(GL_STENCIL_ATTACHMENT, stencilBuffer)
      attach(GL_DEPTH_STENCIL_ATTACHMENT, depthStencilBuffer)
