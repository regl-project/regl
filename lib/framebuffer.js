...
module.exports = function wrapFBOState (
  ...
  function decRef (attachment) {
    if (attachment) {
      if (attachment.texture) {
        attachment.texture.decRef()
      }
      if (attachment.renderbuffer) {
        attachment.renderbuffer.decRef()
      }
    }
  }
  ...
  function refresh (framebuffer) {
    ...
    attach(GL_DEPTH_ATTACHMENT, this.depthAttachment)
    attach(GL_STENCIL_ATTACHMENT, this.stencilAttachment)
    attach(GL_DEPTH_STENCIL_ATTACHMENT, this.depthStencilAttachment)
  ...
  function createFBO (options) {
    ...
    function reglFramebuffer (input) {
      var i, texture, renderbuffer, attachment
      var options = input || {}
      ...
      if ('colorBuffers' in options) {
        ...
        for (i = 0; i < colorBuffers.length; ++i) {
          ...
          texture = head.texture && head.texture._texture
          renderbuffer = head.renderbuffer && head.renderbuffer._renderbuffer
          if (texture) {
        ...
        while (colorBuffers.length < colorCount) {
          if (colorTexture) {
            colorBuffers.push(textureState.create({
              format: colorFormat,
              type: colorType,
              width: width,
              height: height
            ...
            colorBuffers.push(renderbufferState.create({
              format: colorFormat,
              width: width,
              height: height
            }))
      ...
      framebuffer.ownsDepth = ownsDepth
      framebuffer.ownsStencil = ownsStencil
      framebuffer.ownsDepthStencil = ownsDepthStencil

      reglFramebuffer.color = colorBuffers
      reglFramebuffer.depth = depthBuffer
      reglFramebuffer.stencil = stencilBuffer
      reglFramebuffer.depthStencil = depthStencilBuffer
